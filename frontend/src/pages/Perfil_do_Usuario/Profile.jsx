...
import { fetchUserProfile, uploadProfilePhoto } from "../../services/api";
  ...
  const handleImageUpload = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        alert(error.message);
        setImage(profile?.foto_perfil_url || null);
      }
    }
  };
      ...
      <div className="w-full lg:w-1/3 flex flex-col gap-6 lg:gap-8">
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center translate-y-1 transition-transform duration-400 group-hover:-translate-x-[56px]">
            <svg viewBox="40 40 46 40" className="w-[20px] h-[20px] rotate-180 fill-[#f0eeef]">
              <path d="M46 20.038c0-.7-.3-1.5-.8-2.1l-16-17c-1.1-1-3.2-1.4-4.4-.3-1.2 1.1-1.2 3.3 0 4.4l11.3 11.9H3c-1.7 0-3 1.3-3 3s1.3 3 3 3h33.1l-11.3 11.9c-1 1-1.2 3.3 0 4.4 1.2 1.1 3.3.8 4.4-.3l16-17c.5-.5.8-1.1.8-1.9z" />
            </svg>

            <svg viewBox="0 0 46 40" className="w-[20px] h-[20px] rotate-180 fill-[#f0eeef]">
              <path d="M46 20.038c0-.7-.3-1.5-.8-2.1l-16-17c-1.1-1-3.2-1.4-4.4-.3-1.2 1.1-1.2 3.3 0 4.4l11.3 11.9H3c-1.7 0-3 1.3-3 3s1.3 3 3 3h33.1l-11.3 11.9c-1 1-1.2 3.3 0 4.4 1.2 1.1 3.3.8 4.4-.3l16-17c.5-.5.8-1.1.8-1.9z" />
            </svg>
          </div>
        </button>
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 flex flex-col items-center gap-6 shadow-lg">
          ...
          <h2 className="text-2xl font-bold">{profile?.nome || "Usuario"}</h2>
          <p className="text-sm text-gray-300">Cadastro: {profile?.cadastro || "--/--/----"}</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-2/3 flex flex-col gap-6 lg:gap-8">
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 lg:gap-6">
          <div className="bg-white/10 p-6 rounded-xl text-center shadow-md">
            <h3 className="text-3xl font-bold">{profile?.pontos ?? 0}</h3>
            <p className="mt-2">Pontos</p>
          </div>
          <div className="bg-white/10 p-6 rounded-xl text-center shadow-md">
            <h3 className="text-3xl font-bold">{profile?.palpites ?? 0}</h3>
            <p className="mt-2">Palpites</p>
          </div>
          <div className="bg-white/10 p-6 rounded-xl text-center shadow-md">
            <h3 className="text-3xl font-bold">{profile?.posicao ? `${profile.posicao}º` : "-"}</h3>
            <p className="mt-2">Posicao</p>
          </div>
          <div className="bg-white/10 p-6 rounded-xl text-center shadow-md">
            <h3 className="text-3xl font-bold">{profile?.acertos ?? 0}</h3>
            <p className="mt-2">Acertos</p>
          </div>
        </div>

        <div className="bg-white/10 p-6 rounded-xl shadow-md">
          <p className="text-lg" style={{ fontFamily: "sans-serif" }}>Email: {profile?.email || ""}</p>
        </div>

        <div className="bg-white/10 p-6 rounded-xl shadow-md">
          <h3 className="mb-4 font-bold text-lg">Historico de Palpites</h3>
          <ul className="space-y-3 text-sm">
            {(profile?.historico || []).length === 0 && <li>Nenhum palpite registrado.</li>}
            {(profile?.historico || []).map((palpite) => (
              <li key={palpite.id_palpite}>
                {palpite.partida?.time_a} x {palpite.partida?.time_b} - {palpite.escolha}
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          <label
            className="bg-lime-700 px-6 py-3 rounded-xl cursor-pointer hover:scale-105 transition inline-block font-light text-olive-300"
